...
SIMULATE_INVALID_CONFIG = False

def is_outage_simulated():
    return SIMULATE_OUTAGE

def is_invalid_config_simulated():
    return SIMULATE_INVALID_CONFIG
...
def attach_test_routes(app, load_config_fn, base_dir):
    ...
    def get_config_status():
        ...
        return jsonify({
            ...
            'invalidConfigSimulated': SIMULATE_INVALID_CONFIG
        })
    ...
    @app.route('/api/simulate-invalid-config', methods=['GET', 'POST'])
    def toggle_invalid_config():
        global SIMULATE_INVALID_CONFIG
        if request.method == 'POST':
            data = request.get_json() or {}
            if 'simulate' in data:
                SIMULATE_INVALID_CONFIG = bool(data['simulate'])
        return jsonify({'simulateInvalidConfig': SIMULATE_INVALID_CONFIG})
